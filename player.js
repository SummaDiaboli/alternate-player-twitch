		...
		_обновить(оОтменаОбещания, чЧерез) {
			...
			оОбещание.then(() => {
				...
				const оСписокСегментов = РазобратьСписок(false, оВыбранныйВариант.сАбсолютныйАдресСпискаСегментов, сРезультат);
					if (оСписокСегментов.лFMP4 && !г_лFMP4) {
						г_лFMP4 = true;
						...
						м_Загрузчик.Загрузить(оОтменаОбещания, 'GET', г_оИнициализацияFMP4.сАдрес, ЗАГРУЖАТЬ_СПИСОК_СЕГМЕНТОВ_НЕ_ДОЛЬШЕ, null, null, 'сегмент инициализации', false, 0).then(ДобавитьОбработчикИсключений(буф => {
							г_бфИнициализацияFMP4 = буф;
							м_Журнал.Окак(`[Список] Загружен сегмент инициализации fMP4 ${буф.byteLength} байт`);
	...
	function ПреобразоватьСледующийСегмент() {
		...
		for (let оСегмент, чСегмент = 0; оСегмент = г_моОчередь[чСегмент]; ++чСегмент) {
			...
			if (typeof оСегмент.пДанные == 'number' && _оРабочийПоток === null) {
				...
				if (оСегмент.лРазрыв && !г_бфИнициализацияFMP4) {
